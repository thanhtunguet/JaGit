## 2026-06-28
- Fix `@jagit/hook-copilot` transcript parsing to match real VS Code Copilot format: count tool calls from `assistant.message.data.toolRequests[]`, extract `startedAt` from `session.start.data.startTime`; document that token usage/model are not available in Copilot's seat-based billing transcript. 16/16 tests passing. Details: `docs/changelogs/2026-06-28-2230-hook-copilot-real-transcript-format.md`.
- Upgrade `@jagit/hook-copilot` to handle real VS Code Copilot agent `Stop` hook stdin payload (session_id, transcript_path, model, token aggregation from both snake_case and camelCase transcript formats); legacy CLI shell-wrapper mode preserved. 11/11 tests passing. Details: `docs/changelogs/2026-06-28-1231-hook-copilot-vscode-agent-hooks.md`.
- Migrate Anthropic credential to `ANTHROPIC_AUTH_TOKEN` + `ANTHROPIC_BASE_URL`; worker now spawns Claude Code agent with those env vars; config, credentials schema, seed, dashboard Config page, and `.env.example` updated accordingly. Details: `docs/changelogs/2026-06-28-1137-anthropic-auth-token-base-url.md`.
- Implement color-coded status indicators (green for success, blue for progress, orange for pending, red for error) and active station markers on Job Detail timeline (`jobs_.$id.tsx`) in `dashboard-v2`. Details: `docs/changelogs/2026-06-28-1124-timeline-status-indicators.md`.
- Add Events, Console, and Raw Logs tabs to Job Detail page (`jobs_.$id.tsx`) in `dashboard-v2` featuring live event filtering, interactive terminal auto-scroll, and per-step telemetry inspection. Details: `docs/changelogs/2026-06-28-1117-dashboard-v2-job-detail-tabs.md`.
- Wire Jobs overview (`jobs.tsx`) and JobDetail page (`jobs_.$id.tsx`) in `dashboard-v2` to real backend API queries, mutation hooks, and SSE streams; resolve TanStack Router SSR package version mismatches and clean stale node_modules. Details: `docs/changelogs/2026-06-28-1058-dashboard-v2-jobs-integration.md`.
- Implement Overview mode in Historical Codeburn tab (`usage.tsx`) aggregating spend, sessions, API calls, and chart breakdowns across all uploaded users matching old dashboard functionality. Details: `docs/changelogs/2026-06-28-1037-historical-codeburn-overview.md`.
- Implement sleek, glassmorphic login UI (`<AuthGuard>`) in `dashboard-v2` enforcing API token authentication across all routes, listening to global 401 events, and saving tokens to local storage. Details: `docs/changelogs/2026-06-28-1017-auth-guard-login-ui.md`.
- Wire Usage page (`usage.tsx`) in `dashboard-v2` with Live Sessions tab (filtering, KPI cards, category breakdown pie charts, and inspection modal) and Historical CodeBurn tab (uploaded user selection, period filtering, daily spend charts, and model/activity breakdowns). Details: `docs/changelogs/2026-06-28-1009-wire-usage-page.md`.
- Wire MCP Servers page (`mcp-servers.tsx`) in `dashboard-v2` to real backend API queries and mutations with credential reference support. Details: `docs/changelogs/2026-06-28-0959-wire-mcp-servers-page.md`.
- Format credential display names and map MCP server IDs to human-readable names in `dashboard-v2` (`config.tsx`). Details: `docs/changelogs/2026-06-28-0248-format-credential-display-names.md`.
- Wire `approvals.tsx` and `config.tsx` in `dashboard-v2` to real backend API queries, SSE streams, and mutation hooks; replace static mock imports with live updates. Details: `docs/changelogs/2026-06-28-0242-wire-dashboard-v2-approvals-config.md`.

## 2026-06-23
- Consolidate token display format with `formatTokens` utility to format values with `k`, `M`, `B` suffixes and dynamic fractional digits depending on integer length. Details: `docs/changelogs/2026-06-23-1400-format-tokens.md`.

## 2026-06-22
- Fix `log-work` MCP tool to surface Jira worklog failure reasons to the client (`reason` field added to `LogWorkResult`). Details: `docs/changelogs/2026-06-22-1220-log-work-failure-reason.md`.
- Add `log-work` session MCP tool: logs Jira worklog time derived from token cost (67.5 USD = 8h) for a session's already-associated ticket; `createJiraWorklog` now reports real success/failure instead of being fire-and-forget. Details: `docs/changelogs/2026-06-22-1136-session-mcp-log-work.md`.
