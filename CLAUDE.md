...
- **Last completed:** Fixed jobs stuck forever in `runAgent` with Stop/Pause having no effect — `AcpSession.stop()` cleared pending requests' timeout timers without ever calling `reject()` on them, so once the subprocess was killed (by Stop/Pause or the abort-poll loop), any code awaiting `request()`/`runPrompt()`/`start()`'s handshake had no path left to settle and hung indefinitely. `stop()` now rejects every pending request with `"ACP session stopped"` immediately. Added a regression test (`client.test.ts`) using the existing hanging-agent fixture, confirmed RED (10s timeout) before the fix, GREEN after. Also fixed GitLab 400 `source_branch does not exist` on `openMergeRequest` (added `hasChanges` to job state + conditional edge skipping `openMergeRequest`/`jiraWorklog` when the agent made no changes) and `GitlabAdapter.openMergeRequest` swallowing the GitLab error body on non-OK responses. `pnpm --filter @jigit/worker test`: 23 tests passing; `pnpm -r build` clean. Noted (not fixed): `docker-compose.yml` is currently empty on disk, pre-existing uncommitted local change unrelated to these fixes — flag to the user before deploying.
- **In progress:** _n/a_
