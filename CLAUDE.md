...
- **Last completed:** Fixed agent ignoring instructions written in a Jira issue's Description. Root cause: `JiraAdapter.getIssue` (`packages/worker/src/adapters/jira.ts`) reads `description` straight from Jira REST API v3, which returns it as Atlassian Document Format (ADF) — a nested JSON object, not plain text — for any Jira Cloud instance. `IssueData.description` was typed `string` but actually held the raw ADF object at runtime (the Jira response is typed `any`, so TS didn't catch it); `graph.ts`'s `runAgent` then template-string-coerced it into the ACP prompt as the literal text `"Description: [object Object]"`, so the agent never saw the real instructions. Added `adfToText`/`descriptionToText` helpers in `jira.ts` that recursively walk the ADF tree and join text nodes (pass through unchanged if already a string). Regression test in `jira.test.ts` with a realistic ADF fixture (paragraph + bulletList), confirmed RED before the fix, GREEN after. `pnpm --filter @jigit/worker test`: 25 tests passing; `pnpm -r build` clean. Noted (not fixed, out of scope): `packages/api/src/webhooks/normalize.ts`'s `normalizeJira` has the identical `fields.description ?? ""` pattern, but that field isn't read downstream today (`webhooks.service.ts`/`jobs.service.ts` never touch `trigger.description`) so it has no behavioral effect yet — flag if `NormalizedTrigger.description` ever gets consumed (dashboard display, logs, etc.), and consider moving `adfToText`/`descriptionToText` to `packages/shared` at that point so both call sites share one implementation.
- **In progress:** _n/a_
- **Next up:** Restore/regenerate `docker-compose.yml` (currently empty on disk) before any deploy; run `prisma migrate deploy` on deploy (new migration `20260616120000_review_default_false`); investigate pre-existing `webhooks.controller.test.ts` 401 failures (unrelated, found in an earlier session); implement real resume-from-checkpoint for paused jobs (current Pause only halts the in-flight agent run, it doesn't resume LangGraph execution from that point); E2E with real agent session; apply the same ADF→text fix to `normalize.ts` if `NormalizedTrigger.description` ever becomes load-bearing
