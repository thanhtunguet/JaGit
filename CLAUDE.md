...
- **Last completed:** Fixed GitLab 400 `source_branch does not exist` on `openMergeRequest` — root cause was `commitAndPush` only pushing the branch inside its `hasChanges` branch, so a job where the agent made no changes left the worktree branch local-only, and the graph unconditionally proceeded to `openMergeRequest` anyway. Added `hasChanges` to job state and a conditional edge: no changes now routes `commitAndPush → report` directly (status `"done"`, Telegram says "No changes — no MR opened"), skipping `openMergeRequest`/`jiraWorklog`. Also separately fixed `GitlabAdapter.openMergeRequest` swallowing the GitLab API's error detail on non-OK responses (now includes `r.text()` body in the thrown error) so any future GitLab rejection is no longer opaque. Confirmed `No onPostToolUseHook` lines seen in job logs are unrelated, already-known-harmless ACP stderr noise. `pnpm --filter @jigit/worker test`: 22 tests passing; `pnpm -r build` clean.
- **In progress:** _n/a_
