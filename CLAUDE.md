...
- **Last completed:** Merged CodeBurn's Go-based AI usage analytics dashboard into JiGit. Backend: new `UsageModule` (`packages/api/src/usage/`) ports the 3 Go endpoints (upload ZIP, list users, get data) into NestJS — `POST/GET/DELETE /api/usage/*` — using `adm-zip`+`papaparse`+Zod to parse CSVs into a single JSONB blob per upload (`UsageUpload` model, FK to new minimal `User` model) instead of CodeBurn's filesystem CSV storage. Frontend: new `/usage` page replicating CodeBurn's charts/tables (8 ported components under `components/usage/`), plus an "AI Usage" widget on Overview and a new sidebar nav item. `pnpm -r build` clean; `pnpm -r test` passes for shared/api/dashboard except 2 pre-existing `webhooks.controller.test.ts` 401 failures (already flagged below, unrelated) and a worktree-specific `packages/worker` resolution issue. Full session notes: `docs/changelogs/2026-06-20-1200-codeburn-consolidation.md`.
- **In progress:** _n/a_
