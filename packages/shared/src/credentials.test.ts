...
describe("AnthropicCredentialSchema", () => {
  ...
  it("accepts valid anthropic credentials with authToken", () => {
    const result = AnthropicCredentialSchema.parse({
      meta: {},
      secrets: { authToken: "token-xxx" },
    });
    expect(result.secrets.authToken).toBe("token-xxx");
  });
...
describe("credentialSecretKeys", () => {
  ...
  it("returns correct keys for anthropic", () => {
    expect(credentialSecretKeys("anthropic")).toEqual(["authToken", "apiKey"]);
  });
