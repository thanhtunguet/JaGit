...
describe("parseConfig", () => {
  ...
  it("parses ANTHROPIC_AUTH_TOKEN and ANTHROPIC_BASE_URL", () => {
    const { ANTHROPIC_API_KEY, ...rest } = FULL_ENV;
    const cfg = parseConfig({
      ...rest,
      ANTHROPIC_AUTH_TOKEN: "token-123",
      ANTHROPIC_BASE_URL: "https://custom.anthropic.proxy",
    });
    expect(cfg.anthropicAuthToken).toBe("token-123");
    expect(cfg.anthropicBaseUrl).toBe("https://custom.anthropic.proxy");
    expect(cfg.anthropicApiKey).toBe("token-123");
  });
});
