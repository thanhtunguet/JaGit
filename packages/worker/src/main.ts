    ...
    if (useFakeAdapters) {
      ...
      acpRun = async (prompt, onPermission, onOutput, cwd) => {
        ...
        const session = new AcpSession({
          ...
          mcpServers,
          env: {
            ANTHROPIC_AUTH_TOKEN: anthropicCred.secrets["authToken"] ?? anthropicCred.secrets["apiKey"],
            ...(anthropicCred.meta["baseUrl"] ? { ANTHROPIC_BASE_URL: anthropicCred.meta["baseUrl"] } : {}),
          },
