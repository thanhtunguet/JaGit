...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    if (res.status === 401 && typeof window !== "undefined") {
      window.dispatchEvent(new CustomEvent("jigit:auth-unauthorized"));
    }
...
export const setStoredToken = (t: string) => {
  storage.setItem("DASHBOARD_API_TOKEN", t);
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("jigit:auth-updated", { detail: { token: t } }));
  }
};
export const removeStoredToken = () => {
  storage.removeItem("DASHBOARD_API_TOKEN");
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("jigit:auth-unauthorized"));
  }
};
...
export function formatCredentialName(c: { name: string; kind: string }): string {
  ...
  const formattedKind = map[c.kind.toLowerCase()] || c.kind.charAt(0).toUpperCase() + c.kind.slice(1);
  if (!c.name || c.name === "default" || c.name.toLowerCase() === c.kind.toLowerCase()) {
    return formattedKind;
  }
  return `${formattedKind} (${c.name})`;
}
...
// Token helpers are exported at top of file
